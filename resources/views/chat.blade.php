<x-app-layout>
    <div class="left-0 top-0 fixed w-screen h-screen bg-black/50 flex justify-center items-center ">
        <form method="POST" class="flex flex-col gap-4 w-80 h-fit bg-white p-6 rounded-lg" action={{"/messages/$chat->id"}}>
        @csrf
            <div class="relative">
                <h2 class="text-center text-xl font-bold">Add User To Chat</h2>
                <button type="button" class="absolute right-0 top-1/2 -translate-y-1/2 flex gap-2 ml-auto rounded-md p-1 hover:bg-gray-200 group transition cursor-pointer">
            ...
            <div class="flex flex-col flex-grow flex-col bg-white w-full bg-gray-800 overflow-hidden shadow-md sm:rounded-lg">
                <div class="flex flex-col py-8 px-10 text-gray-900 text-gray-100 flex-grow gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <button class="flex gap-2 ml-auto rounded-md px-3 py-2 hover:bg-gray-200 group transition cursor-pointer">
                            Add User
